...
In this section, we try to apply Matrix Factorization to link prediction. 
\\
When we do the experiments, we find a surprising fact: "Lots of people using matrix factorization in the wrong way". And the mistake is still existing in a very recent NIPS paper. To be more surprised, the paper was accepted in KDD 2017 and then submitted in NIPS 2018 uses this incorrect result and being accepted without notice the bug. We then contact the author and surprisingly find that we are the first one to find this bug. Because the paper is already cited by 50 papers, we are really surprised. 
\\
Through the conversation with the author, I found that the author is really smart and very willing to correct the result in the later publication. So, I believe it is still a very good paper with carefully experimentation and the mistake may be not so easy to find. So, we want to point out this mistake in the following section and our final project presentation.   

\subsection{Myth of Matrix Factorization}
Matrix Factorization is extremely popular in solving recommendation problem which can be one of link prediction problem. The only difference is that recommendation problem only focus on the bipartite graph. 

\subsection{Zero Prediction Problem}
